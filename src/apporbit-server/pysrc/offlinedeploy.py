...
import config, utility, resourcefetcher, action
	...
	def get_internal_apporibit_repo_uri(self):
		...
		try:
			if urllib.urlopen(self.internal_repo).getcode() == 200 :
				...
				print "Unable to connect repositories. Check Network settings and enable connection to " + self.internal_repo
		except Exception as exc:
			print "Internal repo not hosted. Please run apporbit-server --setupprovider"
...
''')
		...
		try:
			...
			ao_repo_file = os.open("/etc/yum.repos.d/apporbit-offline.repo",flags)
		except OSError as e:
	...
	def set_iptables(self):
		...
		command = "iptables -D INPUT -j REJECT --reject-with icmp-host-prohibited && iptables -D  FORWARD -j REJECT --reject-with icmp-host-prohibited"
		return_code, out, err = self.utility_obj.cmdExecute(command, "", show=True)
		if not return_code:
			return False
		return True
	...
	def uncompress_resources(self):
		...
		self.AO_PACKAGES_PATH = raw_input("Default(" + self.CWD + "appOrbitPackages.tar.gz): ") or self.CWD + "appOrbitPackages.tar.gz"
		...
		return_code, out, err = self.utility_obj.cmdExecute(command, " Untar appOrbitPackages.tar.gz", show=True)
		if not return_code:
	...
	def add_chef_port_to_firewall(self):
		if os.path.isfile("/usr/bin/firewall-cmd"):
			...
			command = "firewall-cmd --permanent --add-port=" + str(self.chef_port) + "/tcp"
			return_code, out, err = self.utility_obj.cmdExecute(command, "", show=True)
			if not return_code:
				print "Chef port not added to firewall :- " + str(err)
		print "Saving Iptables"
		return_code, out,err = self.utility_obj.cmdExecute("/sbin/service iptables save", "", show=True)
		'''
		print "Loading Chef Server..."
		self.docker_obj.docker_load(self.TMPDIR + "appOrbitPackages/apporbit-chef:2.0.tar")

		print "Starting Chef service.."
		vol_mount = {
			"/var/lib/apporbit/chef-server":"/var/opt/chef-server:Z",
			"/opt/apporbit/key/":"/var/opt/chef-server/nginx/ca/:Z",
			"chef-conf":"/etc/chef-server/:Z"
		}
		other_flags = "-m 2g -it --restart=always -e UPGRADE=" + self.chef_upgrade +"-p " + str(self.chef_port) + ":" + str(self.chef_port)
		self.docker_obj.docker_run("apporbit-chef","apporbit/apporbit-chef:2.0", vol_mount, other_flags, show=True)
		'''
	def set_config_info(self):
		emailid = "admin@apporbit.com"
		onPremMode = True

		self.emailid = raw_input("Enter the User Email id for On Prem Deployment. Default(" + emailid + "):") or emailid
		offline_mode = "true"
	...
	def generate_ssl_certs(self):
		if not os.path.isfile("/opt/apporbit/key/apporbitserver.key") or not os.path.isfile("/opt/apporbit/key/apporbitserver.crt"):
			print "1) use existing certificate"
			print "2) Create a self signed certificate"
			ssltype = raw_input("Enter the type of ssl certificate [Default:2]:") or 2
			if ssltype == 2:
				#Generate SSL Certiticate for https and put it in a volume mount controller location.
				command = 'openssl req -new -newkey rsa:4096 -days 365 -nodes -x509 -subj "/C=US/ST=NY/L=appOrbit/O=Dis/CN=www.apporbit.com" -keyout /opt/apporbit/key/apporbitserver.key -out /opt/apporbit/key/apporbitserver.crt'
				return_code, out, err = self.utility_obj.cmdExecute(command, "", show=True)
				if not return_code:
					print " openssl certificate generation failed :- " + str(err)
					return False
				return True
			else:
				print "Rename your certificate files as apporbitserver.crt and key as apporbitserver.key"
				sslkeydir = raw_input("Enter the location where your certificate and key file exist:")
				if not os.path.exists(sslkeydir):
					print "Dir does not exist, Exiting..."
					return False

				os.chdir(sslkeydir)
				if not os.path.isfile("apporbitserver.key") or not os.path.isfile("apporbitserver.crt"):
					print "key and certificate files are missing."
					print "Note that key and crt file name should be apporbitserver.key and apporbitserver.crt. Rename your files accordingly and retry."
					return False
				src = os.getcwd()
				dest = "/opt/apporbit/key/"
				shutil.copyfile(src + "apporbitserver.key", dest + "apporbitserver.key")
				shutil.copyfile(src + "apporbitserver.crt", dest + "apporbitserver.crt")

	def clean_setup_maybe(self):
		print "Do you want to clean up the setup (removes db, Rabbitmq Data etc.,) ?"
		print "press 1 to clean the setup."
		print "press 2 to retain the older entries.."
		cleanSetup = raw_input("Default(2):") or 2

		if cleanSetup == 1:
			command = 'rm -rf "/var/lib/apporbit/" && rm -rf "/var/log/apporbit/" && rm -rf "/opt/apporbit/"'
			return_code, out, err = self.utility_obj.cmdExecute(command, "", show=True)
			self.chef_upgrade = 1
		else:
			self.chef_upgrade = 2
		...
		command = 'touch -a "/opt/apporbit/conf/apporbit.ini" && chcon -Rt svirt_sandbox_file_t /opt/apporbit/conf/apporbit.ini'
		self.utility_obj.cmdExecute(command, "", show=True)
	...
	def remove_conflicting_containers(self):
		...
		for container in containers:
			...
			if out:
				print "Container " + container + " is already runnning. The container will removed first."
				return_code, out, err = self.utility_obj.cmdExecute("docker rm -f " + container, "", show =True)
				if not return_code:
        ...
        def setup_docker_daemon_insecure_reg(self):
                ...
                for line in fileinput.input(docker_config_path, inplace=True):
                        if re.compile(r'^INSECURE_REGISTRY.*').match(line):
                                flag = True
                        line = re.sub(r'^INSECURE_REGISTRY.*',"INSECURE_REGISTRY='--insecure-registry " + self.internal_docker_reg + "'", line.rstrip())
                        print line

                if not flag:
                        with open(docker_config_path, "a") as myfile:
                                myfile.write("INSECURE_REGISTRY='--insecure-registry " + self.internal_docker_reg + "'")
	...
	def load_containers(self):
		...
		for container in containers:
			self.docker_obj.docker_load(path + container + ".tar")
		self.docker_obj.docker_tag(rf.hub_images, "", self.internal_docker_reg + '/')
		self.docker_obj.docker_push(rf.hub_images, self.internal_docker_reg + '/')
...
# Following options are truely optional. They will default to following defaults
[Deployment Setup]
remove_data = False
apporbit_deploy = all
deploy_chef = True
...
''')
		...
		shutil.copyfile(self.CWD + 'docker-compose', config_obj.APPORBIT_BIN + '/docker-compose')
		self.utility_obj.cmdExecute("chmod a+x " + config_obj.APPORBIT_BIN + '/docker-compose', "", True)
