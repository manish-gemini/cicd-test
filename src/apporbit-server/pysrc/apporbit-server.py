...
def main():
    ...
    if  args.setuponly or (setupRequired and not skipSetup):
        ...
        print "appOrbit server is already configured. Run the installer again with --upgrade if you wish to upgrade. For example:\n" 
	print "bash <(curl -s  http://repos.apporbit.com/install/appOrbitKit/install.sh) --upgrade \n"
        action_obj.showStatus(config_obj,show=True)
        print ""
        parser.print_help()
